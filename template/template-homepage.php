...
$self_id = logic_main(get_the_ID(), false);
get_header();
?>
<?php while (have_posts()) : the_post(); ?>

    <!-- Intro Content begns -->
    <div id="<?php echo esc_attr(logic_sectionID(logic_main($post->ID, false))); ?>" class="onepage-home ">
        <?php the_content(); ?>
    </div>
...
if (isset($menu_locations['primary'])) :
    ...
    foreach ($menu_items as $item) {
        if (($item->object == 'page') && (logic_get_post_meta(logic_main($item->object_id, false), 'xs_page_section') == "on") && ($item->object_id != $self_id))
    ...
    if ($query->have_posts()):
        while ($query->have_posts()):
            $query->the_post();
            $section_id = logic_sectionID(logic_main($post->ID, false));
            ?>
            <div id="<?php echo esc_attr($section_id); ?>" class="section-wraper">
                <?php the_content(); ?>

                <!-- Footer edit section -->
                <?php echo logic_edit_section(); ?>
            </div>
